...
#------------------------------------------------------------------
def createDirs (inputDir, pdbsDir, outputDir):
	def checkExistingDir (outputDir):
		if os.path.lexists (outputDir):
			headDir, tailDir = os.path.split (outputDir)
			oldDir = os.path.join (headDir, "old-" + tailDir)
			if os.path.lexists (oldDir):
					checkExistingDir (oldDir)
			os.rename (outputDir, oldDir)
	...
	for filename in files:
		sourceFilename	= "%s/%s" % (os.path.abspath(inputDir), filename)
